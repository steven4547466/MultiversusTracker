...
async function handleMatchHistory(interaction, id, name, type, currentPage, page, startAt, goingBack = false, mobile = false) {
  if (mobile == null) mobile = false
  if (!goingBack && page > 0) {
    if (!cache[interaction.message.id]) cache[interaction.message.id] = {
      timeout: setTimeout(() => {
        delete cache[interaction.message.id]
      }, 300000), pages: []
    }
    else {
      clearTimeout(cache[interaction.message.id].timeout)

      cache[interaction.message.id].timeout = setTimeout(() => {
        delete cache[interaction.message.id]
      }, 300000)
    }
    cache[interaction.message.id].pages[page - 1] = { content: interaction.message.content, embeds: interaction.message.embeds }
  }
  else if (goingBack) {
    if (!cache[interaction.message.id]) {
      interaction.editReply({ content: "This match history has timed out." })
      return;
    }
    else {
      clearTimeout(cache[interaction.message.id].timeout)
      ...
      return interaction.editReply({ content: cache[interaction.message.id].pages[page].content, embeds: cache[interaction.message.id].pages[page].embeds, components: [row] })
    }
  }
  ...
  if (!mobile) {
    let description = `\`\`\`ansi\n[1;37mPage: ${page + 1}\n[1;37mMatch history for ${name}${type ? ` (${type})` : ""}[0m\n`;
    ...
    for (let match of matches) {
      ...
      if (!matchData.server_data || !matchData.server_data.PlayerData) continue
    ...
    const row = new ActionRowBuilder()
      .addComponents(
        ...
        new ButtonBuilder()
          ...
          .setLabel('Next Page')
          .setStyle(ButtonStyle.Primary)
          .setDisabled(!moreAvailable),
      );

    interaction.editReply({ content: description + `${name};${id};${page};${currentPage};${startAt};${type};${mobile}\`\`\``, components: [row] })
  } else {
    let embed = new EmbedBuilder()
      .setTitle(`Match history for ${name}${type ? ` (${type})` : ""}`)
      .setColor("#00ffff")
      .setThumbnail("https://cdn.discordapp.com/attachments/501853552835297285/1003349507947384912/Multiversus_long_logo_1.png")
      .setFooter({ text: `Page ${page + 1} | Created by Steven4547466#1407`, iconURL: interaction.client.user.displayAvatarURL() });
    for (let match of matches) {
      let matchData
...
`
    ...
    interaction.editReply({ content: `\`\`\`\n${name};${id};${page};${currentPage};${startAt};${type};${mobile}\`\`\``, embeds: [embed], components: [row] })
  }
...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    .setName("getmatchhistory")
    .setDescription("Get the match history of a user.")
    .addStringOption(
      opt =>
        opt.setName("user")
    ...
    .addStringOption(
      opt =>
        opt.setName("type")
          .setDescription("The type of match history to get")
          .setRequired(false)
          .addChoices(
            { name: '1v1s', value: '1v1' },
            { name: '2v2s', value: '2v2' },
    ...
    .addBooleanOption(
      opt =>
        opt.setName("mobile")
          .setDescription("Display a mobile version of the match history")
          .setRequired(false)
    ),
  handler: async (interaction) => {
    ...
    handleMatchHistory(interaction, user.id, name, type, 1, 0, 0, null, mobile)
  }
}